...
THUMBNAIL_SIZE = 32
# fmt: off
KEY = [0x88, 0xE3, 0x4F, 0x8F, 0x08, 0x17, 0x79, 0xF1, 0xE9, 0xF3, 0x94, 0x37, 0x0A, 0xD4, 0x05, 0x89] #prolly change this too
# fmt: on

key_size = len(KEY)
block_size = 8    #Prolly changethis



# def tea_encrypt(plaintext: bytes):
#     ciphertext = 0
...
source_files = (__file__, "tea.py")

#Sending and receiving part common for all
if (HOST := os.getenv("RECEIVER")) is not None:
    ...
    C = cryterion.encrypt(
        lambda plaintext: encrypt(plaintext,KEY),
        P,
        key_size,
        block_size,
    ...
    D = cryterion.decrypt(
        lambda ciphertext: decrypt(ciphertext,KEY),
        C,
        key_size,
        block_size,
        cryterion.code_size_from_files(source_files),
