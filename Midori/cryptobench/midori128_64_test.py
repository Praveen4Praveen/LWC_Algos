from Midori128_64.encrypt import encrypt as midori128_64_encrypt
from Midori128_64.decrypt import decrypt as midori128_64_decrypt
import os
...
def encrypt_wrapper(plaintext: bytes):
    ...
    for i in range(0, len(plaintext), block_size):
        ...
        state = midori128_64_encrypt(block.decode("charmap"), key, block_size * 8)
...
def decrypt_wrapper(ciphertext: bytes):
    ...
    for i in range(0, len(ciphertext), block_size):
        ...
        state = midori128_64_decrypt(state, key, block_size * 8)
        plaintext.extend(state[i][j] for i in range(4) for j in range(4))

    return bytes(plaintext)


source_files = (__file__, "Midori128_64/encrypt.py", "Midori128_64/decrypt.py")

if (HOST := os.getenv("RECEIVER")) is not None:
    # HOST = "192.168.166.32"
    PORT = 8000

    P = cryterion.random_text(10_000)
